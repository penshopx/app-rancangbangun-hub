...
import { Users, Shield, Award, TrendingUp, RefreshCw, Star, CheckCircle, AlertTriangle, Building2, Briefcase } from 'lucide-react';
...
const LPJK_DECODE = {
  ...
  "BG003": "Gedung Industri",
  "BG004": "Gedung Perbelanjaan",
  "BG005": "Gedung Kesehatan",
  "BG006": "Gedung Pendidikan",
  "BS001": "Jalan",
  "BS002": "Jembatan & Fly Over",
  "KK008": "Perkerasan Aspal",
  "BG009": "Gedung Lainnya"
};

const ContractorRegistryEnhanced = () => {
  ...
  const seedData = async () => {
    try {
      ...
      fetchContractors();
      alert('✅ Sample contractors dengan LPJK classification loaded!');
    } catch (error) {
      console.error('Error:', error);
    }
  };
    ...
    <div className="p-8">
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-600">Database Kontraktor Terverifikasi LPJK - 73 Klasifikasi</p>
          </div>
        </div>
      ...
      <div className="mb-6 bg-white rounded-lg shadow-md p-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <label className="flex items-center gap-2 px-4 py-2 border border-slate-300 rounded-lg cursor-pointer hover:bg-slate-50">
            ...
            <span className="text-sm font-semibold text-slate-700">✅ Verified LPJK Only</span>
          </label>

          <div className="px-4 py-2 bg-blue-50 border border-blue-200 rounded-lg text-center">
            <p className="text-xs font-semibold text-blue-800">📋 73 LPJK Classifications</p>
            <p className="text-xs text-blue-600">BG, BS, IN, KK, PB, PL</p>
          </div>
        </div>
      </div>
      ...
      {contractors.length === 0 ? (
        <div className="bg-white rounded-lg shadow-md p-12 text-center">
          ...
          <p className="text-slate-500 mb-4">Klik "Load Sample" untuk memuat data LPJK-classified contractors</p>
        </div>
      ) : (
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    {contractor.is_verified && (
                      <CheckCircle className="w-5 h-5 text-green-500" title="Verified LPJK" />
                    )}
                  </div>
                  <p className="text-sm text-slate-600">LPJK: {contractor.license_number}</p>
                  
                  {/* LPJK Classifications */}
                  <div className="flex flex-wrap gap-2 mt-3">
                    {contractor.specialization.map((spec, idx) => (
                      <span key={idx} className="text-xs bg-indigo-100 text-indigo-700 px-2 py-1 rounded font-semibold flex items-center gap-1">
                        <Building2 className="w-3 h-3" />
                        {spec} - {LPJK_DECODE[spec] || spec}
                      </span>
                    ))}
                  </div>
                </div>
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {selectedContractor.specialization.map((spec, idx) => (
                      <span key={idx} className="text-xs bg-indigo-50 border border-indigo-200 text-indigo-700 px-3 py-1 rounded-full font-semibold">
                        {spec} - {LPJK_DECODE[spec] || spec}
                      </span>
                    ))}
                  </div>
...
export default ContractorRegistryEnhanced;
